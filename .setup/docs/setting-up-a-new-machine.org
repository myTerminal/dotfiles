#+TITLE: Setting Up a New Machine
#+AUTHOR: Mohammed Ismail Ansari
#+OPTIONS: ^:{}

* Choice of Operating System

  | Purpose                          | Operating System |
  | Gaming with proprietary hardware | Windows          |
  | Gaming with hybrid graphics      | Pop!_OS          |
  | Secondary travel setup           | Nobara           |
  | Primary setup                    | Void Linux       |

* Storage Drive Partition Map

Partition Table Type: GPT

  | Volume    | Size | Type  | BIOS (SATA) | UEFI (SATA) | UEFI (NVMe) |
  | BIOS BOOT | 1M   | --    | sda1        | --          | --          |
  | EFI       | 1G   | FAT32 |             | sda1        | nvme0n1p1   |
  | SWAP      | ?    | SWAP  | sda2        | sda2        | nvme0n1p2   |
  | Boot      | 2G   | EXT4  | sda3        | sda3        | nvme0n1p3   |
  | Root      | ?    | LUKS  | sda4        | sda4        | nvme0n1p4   |

* Steps

- /(Optional)/ Install Windows with absolute basic settings and minimal software, run updates a few tens of times
  - (Optional) Install and configure Waterfox
- Install Linux
  - If installing from within a LIVE environment, create an SSH key and add it to my GitHub and GitLab account
- Init [[https://github.com/myTerminal/dotfiles][dotfiles]] using [[../../bootstrap][the bootstrap script]] and trigger forge
- Create an add-on setup script for the specific hardware:
  - Test/Fix display scaling
  - Install graphics drivers
  - Test/Fix bluetooth connectivity
  - Test/Fix Sound
  - Test/Fix Microphone
  - Test/Fix mouse and trackpad
  - Test/Fix camera
  - Configure power management
- Configure software and services
  - Keepass
  - Waterfox
    - Choose Dark color theme on startup
    - Set Brave Search as the search engine
    - Configure behavior around tab cycling
    - Switch to Vertical Tabs
    - Create tab containers, check "Select a container for each new tab"
    - Installing web extensions
      - uBlock Origin
      - KeePassXC-Browser
      - Raindrop.io
      - Firefox Multi-Account Containers
      - Temporary Containers Plus
      - Emoji
    - Set up pinned tabs, logins, etc.
    - Customize sidebar
    - Customizing toolbar
  - Betterbird
    - Configure Email
    - Configure Calendars
    - Configure Addressbooks
  - LogSeq
  - Obsidian
  - Thunar (may not be needed anymore)
  - Darktable
  - Steam
  - pCloud
  - Bluetooth
  - Calibre
    - KePub Input
      KePub Metadata Reader
      KePub Metadata Writer
      KePub Output
      KoboTouchExtended
- Verify software packages
- Test/Improve scripts under [[../hardware][~/.setup/hardware]] and [[../../.bin][~/.bin]]
- Verify reference of the machine at/under the following:
  - Home network as a new device
    - Static IP assignment
  - GitHub and GitLab (SSH keys)
  - Syncthing as a node
  - KeePass database as a key
  - Telegram
- Create maintenance tasks in TickTick
- Add entry in network-map.conf
- Add entry in *machines* spreadsheet

# Local Variables:
# eval: (visual-line-mode)
# End:
